...
from apex_sam.data.chaos_mr import case_id_from_path, iter_cases, iter_label_slices, load_case
...
def _load_support_for_label(match, label_value: int):
    image_volume, label_volume = load_case(match.meta.slice_path, match.meta.label_path)
...
def run_evaluation(config: ApexConfig) -> RunSummary:
    ...
    for label_value in config.test_labels:
        ...
        for case_index, (image_path, label_path) in enumerate(iter_cases(config.data_dir)):
            ...
            image_volume, label_volume = load_case(image_path, label_path)
            ...
            for slice_id in valid_slices:
                ...
                for match in results:
                    support_image, support_mask = _load_support_for_label(match, int(label_value))
                    if np.asarray(support_mask).sum() <= 0:
                        continue
                    entry = {'match': match, 'image': support_image, 'mask': support_mask}
                    valid_candidates_all.append(entry)
                    support_case = case_id_from_path(match.meta.slice_path)
                    if config.retrieval_skip_self and support_case == case_id and int(match.meta.slice_index) == int(slice_id):
                        continue
                    valid_candidates.append(entry)
                if not valid_candidates:
                    if valid_candidates_all:
                        logger.warning('[Case %s][Slice %s] Only self-matching supports found; falling back to allow self.', case_id, slice_id)
                ...
                records.append({
                    ...
                    'dice': float(dice),
                    'support_case': case_id_from_path(match.meta.slice_path),
                    'support_slice': int(match.meta.slice_index),
    ...
    return RunSummary(run_dir=run_dir, num_cases=seen_cases, num_slices=seen_slices, mean_dice=float(dice_summary['overall_mean_dice']), metrics_csv=metrics_csv, summary_json=summary_json)
